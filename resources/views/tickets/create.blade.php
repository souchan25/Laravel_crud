        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="overflow-hidden rounded-2xl border border-zinc-200/90 bg-white/90 shadow-premium-lg backdrop-blur-sm">
                <div class="border-b border-zinc-200/80 bg-zinc-50/80 px-6 py-4">
                    <p class="text-sm text-zinc-600">{{ __('Add a new item to the board.') }}</p>
                </div>
                <div class="p-6 sm:p-8">
                    <form method="POST" action="{{ route('tickets.store') }}" class="space-y-6">
                        ...
                        <div>
                            ...
                            <x-text-input id="title" class="mt-1 block w-full" type="text" name="title" required :value="old('title')" autofocus />
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="5" required
                                class="mt-1 block w-full border-zinc-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-lg shadow-sm">{{ old('description') }}</textarea>
                        ...
                        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                            <div>
                                ...
                                <select id="type" name="type" class="mt-1 block w-full border-zinc-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-lg shadow-sm">
                                    <option value="bug" {{ old('type') == 'bug' ? 'selected' : '' }}>{{ __('Bug') }}</option>
                                    <option value="feature" {{ old('type') == 'feature' ? 'selected' : '' }}>{{ __('Feature') }}</option>
                                </select>
                            </div>
                            <div>
                        ...
                        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                            <div>
                                ...
                                <select id="status" name="status" class="mt-1 block w-full border-zinc-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-lg shadow-sm">
                                    <option value="open" {{ old('status', 'open') == 'open' ? 'selected' : '' }}>{{ __('Open') }}</option>
                                    <option value="in-progress" {{ old('status') == 'in-progress' ? 'selected' : '' }}>{{ __('In progress') }}</option>
                        ...
                        <div class="flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between pt-2">
                            <a class="text-center text-sm font-semibold text-indigo-600 hover:text-indigo-800" href="{{ route('tickets.index') }}">
                                {{ __('Cancel') }}
                            </a>
